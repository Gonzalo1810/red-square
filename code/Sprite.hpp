...
#define SPRITE_HEADER
    ...
    namespace example
    {
        ...
        public:
            ...
            /// Recive la nueva velocidad en valor absoluto
            void set_global_speed(const float new_speed)
            {
                if(this->speed.coordinates.x() < 0)
                    speed.coordinates.x() = -new_speed;
                else
                    speed.coordinates.x() = new_speed;

                if(this->speed.coordinates.y() < 0)
                    speed.coordinates.y() = -new_speed;
                else
                    speed.coordinates.y() = new_speed;
            }
